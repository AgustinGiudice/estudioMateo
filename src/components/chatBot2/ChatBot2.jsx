...
const derechoData = [
  {
    ...
    contenido: "Sucesiones - Conflictos familiares - Cuota alimentaria - Comunicación con los hijos - Divorcio",
  ...
  },
  {
    ...
    contenido: "Despidos - Accidentes de trabajo - Trabajo en condiciones insalubres",
  ...
  {
    area: "Derecho Civil",
    contenido: "Contratos - Daños y perjuicios - Derechos reales - Derecho de familia",
  ...
  return (
    <div className={`chat-bot ${chatOpen ? 'open' : 'closed'}`}>
      <div className="chat-header" onClick={() => setChatOpen(!chatOpen)}>
        ...
        <img src={bot} alt="Bot" className={`bot-icon ${chatOpen ? '' : 'bot-closed'}`} />
      </div>
      <div className="messages">
        {messages.map((msg, index) => (
          <div key={index} className={`message ${msg.from}`}>
            {msg.text}
            {msg.options && msg.options.map((option, i) => (
              <>
                <br key={`br-${i}`} /> 
                <button key={i} onClick={option.action} className="option-button">
                  {option.text}
                </button>
              </>
            ))}
      ...
      {chatOpen && (
        <form className="message-form" onSubmit={(e) => e.preventDefault()}>
          <input type="text" name="message" placeholder="Escribe tu mensaje aquí..." disabled />
        </form>
      )}
    </div>
  );
